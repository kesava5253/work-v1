...
Set Limit :								
<select name="limit" id="limit" onChange="window.location='search_db.php?setlimit='+this.value"  >
...
if(isset($_GET["page"])){
	...
	$page=(int)$page;		
			$count_result=$dbh->prepare("Select count(*) as c from userdata , UPLOADS where userdata.Id = UPLOADS.Id group by userdata.Id");
			$count_result->execute();
			
			$result=$dbh->prepare("Select * From userdata Order By $order limit $page1,$limit");
			$result->execute();
while($row = $result->fetch(PDO::FETCH_ASSOC))
	{
		$rows=$count_result->fetch(PDO::FETCH_ASSOC);
 		$Id=$row['Id'];
 		$user=$row['username'];
 		$mail=$row['email'];
		$status=$row['status'];
	...
	?>	
		<tr>
           <td><?php echo $Id ?></td>
    <?php	
      echo "<td><a href='user.php?userid=$Id' style='color: #cc0000'>$user</a></td>";
	?>
            <td><?php echo $mail ?></td>
  <?php	
	echo "<td><a href='password_change.php?userid=$Id'>Change Password</a></td>";
		
    echo "<td><a href='user_docs.php?userid=$Id'> $doc</a></td>";
if($status == '1'){
	    echo "<td><a href='update_status.php?userid=$Id'>Active</a></td>";
	}
else{
	echo "<td><a href='update_status.php?userid=$Id'>Deactivated</a></td> </tr>";
	}
}
